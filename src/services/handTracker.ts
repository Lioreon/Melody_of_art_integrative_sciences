...
export class HandTracker {
  ...
  public async initialize(video: HTMLVideoElement, onResults: HandTrackerCallback,
    mode: TrackingModeType = 'hands', onError?: () => void, deviceId = ''): Promise<boolean> {
    this.stop();
    this.lastError = '';
    ...
    try {
      const stream = await navigator.mediaDevices.getUserMedia({
        audio: false, video: { ...(deviceId ? { deviceId: { exact: deviceId } } : {}), width: { ideal: 640 }, height: { ideal: 480 }, frameRate: { ideal: 30, max: 30 } }
      });
      ...
      for (const track of stream.getVideoTracks()) {
        track.onended = () => {
          ...
          this.stop();
          this.callback?.(trackingState(null, null));
          this.onError?.();
        };
      }
      if (mode === 'colored_balls') {
        ...
        this.colors.start(video, state => {
          if (generation === this.generation) this.callback?.(state);
        });
        return true;
      }
      ...
      if (generation === this.generation) {
        ...
        this.lastError = name === 'NotAllowedError'
          ...
          : 'No se pudo iniciar la cámara o el modelo de seguimiento. Prueba otra fuente o el modo de pelotas.';
  ...
  private loop = async (now: number) => {
    ...
    if (!document.hidden && video.readyState >= 2 && video.currentTime !== this.lastVideoTime && now - this.lastFrameTime >= 1000 / 30) {
    ...
    if (generation === this.generation && this.isRunning) this.frameId = requestAnimationFrame(this.loop);
  };
  ...
  private processResults(results: Results) {
    ...
    // Spatial ordering avoids overwriting a hand when model labels are ambiguous.
    palms.sort((a, b) => a.center.x - b.center.x);
    this.callback?.(trackingState(palms[0] ?? null, palms[1] ?? null,
      this.videoElement?.videoWidth || 640, this.videoElement?.videoHeight || 480));
  }
  ...
  public setSimulatedDistanceCm(distance: number) {
    ...
    this.setSimulatedPalmPositions(0.5 - half, this.simLeftY, 0.5 + half, this.simRightY);
  }

  private emitSimulationState() {
    this.callback?.(trackingState(palmAt({ x: this.simLeftX, y: this.simLeftY }),
  ...
  public stop() {
    ...
    this.hands = null;
    if (hands) void (this.pending ?? Promise.resolve()).catch(() => {}).then(() => hands.close()).catch(() => {});
    this.pending = null;
